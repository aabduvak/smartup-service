...
def create_deals(branch_id: str, date: str):
    ...
    try:
        ...
        for deal in deals:
            ...
            date_of_shipment = datetime.strptime(
                info["date_of_shipment"], "%d.%m.%Y"
            ).date()
            date_of_order = datetime.strptime(
                info["date_of_order"], "%d.%m.%Y %H:%M:%S"
            ).date()
            ...
            if "payment_type_id" in info:
                payment_type = PaymentType.objects.get(
                    smartup_id=info["payment_type_id"]
                )
