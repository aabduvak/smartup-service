...
def create_regions():
    columns = ["region_id", "name", "lat_lng"]
...
def create_cities():
    ...
    for region in regions:
        ...
        response = get_data(
            endpoint="/b/anor/mr/region_list+cities&table",
            columns=columns,
            parent=region.smartup_id,
        )
        if not response:
            return None
        ...
        try:
            for city in cities:
                if not City.objects.filter(smartup_id=city[0]).exists():
                    City.objects.create(smartup_id=city[0], name=city[1], region=region)
        except:
            return None
    return True
...
def create_districts():
    ...
    for city in cities:
        ...
        response = get_data(
            endpoint="/b/anor/mr/region_list+towns&table",
            columns=columns,
            parent=city.smartup_id,
        )
        if not response:
            return None
        ...
        try:
            for town in towns:
                if not District.objects.filter(smartup_id=town[0]).exists():
                    District.objects.create(smartup_id=town[0], name=town[1], city=city)
        except:
            return None
    return True
